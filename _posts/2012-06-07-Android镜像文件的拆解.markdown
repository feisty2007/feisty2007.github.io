...
userdata.img挂载到 /Asserts/Images/ 下的镜像，它包含了应用及用户相关的数据。
...
* **userdata.img** 只用于使用了 -wipe-data参数时。通常将~/.android/userdata-qemu.img（linux下）作为 /Asserts/Images/ 分区镜像挂载，而使用 -wipe-data 时会复制userdata.img中的内容到userdata-qemu.img。
